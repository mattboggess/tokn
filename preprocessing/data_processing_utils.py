...
def tag_terms(text, terms, nlp=None, invalid_pos=[], invalid_dep=[], expand_np=False):
   ...
   """
    ...
    for spacy_term in terms:
        ...
        # skip really short acronyms
        # TODO: Handle these better 
        if '-pron-' in term_lemma or len(term_lemma) == 1:
            continue
        
        # save time by skipping words whose components are not all present
        heur_term = [HEURISTIC_MAPPING[l] if l in HEURISTIC_MAPPING else l for l in lemma_term_span]
        heur_text = [HEURISTIC_MAPPING[l] if l in HEURISTIC_MAPPING else l for l in lemmatized_text]
        if not all([l in lemmatized_text for l in lemma_term_span]) and \
           not all([l in heur_text for l in heur_term]):
            continue
