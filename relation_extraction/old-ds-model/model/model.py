...
class BagAttentionBert(BaseModel):
    ...
    def __init__(self, num_classes, dropout_rate=0.3):
        ...
        self.fc_e1 = nn.Linear(bert_config["hidden_size"], bert_config["hidden_size"])
        self.fc_e2 = nn.Linear(bert_config["hidden_size"], bert_config["hidden_size"])
        ...
        self.fc_class = nn.Linear(bert_config["hidden_size"] * 2, num_classes)
    ...
    def init_weights(self):
        ...
        nn.init.kaiming_uniform_(self.fc_e1.weight)
        nn.init.kaiming_uniform_(self.fc_e2.weight)
    ...
    def forward(self, batch_data, evaluate=True):
        ...
        #e1_avg = self.fc_e1(self.dropout(e1_avg)).relu()
        #e2_avg = self.fc_e2(self.dropout(e2_avg)).relu()
        
        # concatenate sentence level, e1, and e2
        #rel_rep = torch.cat([cls_output, e1_avg, e2_avg], dim=-1)
        rel_rep = torch.cat([e1_avg, e2_avg], dim=-1)
